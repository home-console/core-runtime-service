...
async def e2e_test_dual_mode_workflow():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        try:
            ...
            print(
                f"  Vault storage: {dual_config.vault_storage_type} ({dual_config.vault_db_path})"
            )
            ...
            from modules.storage.errors import NamespaceViolationError

            try:
                # Attempt to write vault namespace to core (should fail)
                await dual_manager.set(
                    "secrets.store", "test_key", {"test": "value"}, target="core"
                )
                print("✗ NamespaceViolationError NOT raised (BUG!)")
                return False
            except NamespaceViolationError:
                print(
                    "✓ Namespace enforcement working (vault namespace blocked from core)"
                )
            ...
            print("  • Namespace enforcement prevents cross-storage writes")
