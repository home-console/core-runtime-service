...
async def test_backward_compat_old_adapter_api():
    ...
    try:
        ...
        assert value1 == {"value": "test"}, f"Expected {{'value': 'test'}}, got {value1}"
        assert value2 == {"pwd": "secret"}, f"Expected {{'pwd': 'secret'}}, got {value2}"
        ...
        assert "app.config" in namespaces, f"Expected 'app.config' in namespaces, got {namespaces}"
...
async def test_single_mode_storage_manager():
    ...
    try:
        ...
        assert value1 == {"value": "test2"}, f"Expected {{'value': 'test2'}}, got {value1}"
        assert value2 == {"key": "secret"}, f"Expected {{'key': 'secret'}}, got {value2}"
...
async def test_dual_mode_isolation():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        try:
            ...
            assert vault_pwd == {"pwd": "secret"}, f"Expected secret in vault, got {vault_pwd}"
            ...
            assert core_setting == {"val": "x"}, f"Expected setting in core, got {core_setting}"
...
async def test_config_backward_compat():
    ...
    try:
        ...
        assert config.storage_mode == "single", f"Expected 'single', got {config.storage_mode}"
...
async def test_single_to_dual_migration():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        try:
            ...
            assert migrated_count == 2, f"Expected 2 records migrated (secrets.store + oauth.tokens), got {migrated_count}"
            ...
            assert vault_api_key == {"key": "secret123"}, f"API key not in vault: {vault_api_key}"
            assert vault_token == {"token": "ghp_xxx"}, f"Token not in vault: {vault_token}"
            
            # Core records should still be in core storage
            core_feature = await dual_manager.get("app.config", "feature")
            assert core_feature == {"enabled": True}, f"Feature not in core: {core_feature}"
...
async def main():
    ...
    # Run tests
    results.append(("Old StorageAdapter API", await test_backward_compat_old_adapter_api()))
    results.append(("StorageManager Single Mode", await test_single_mode_storage_manager()))
