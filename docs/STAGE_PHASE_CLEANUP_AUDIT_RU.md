...
## 1. Цель
Убрать из production-кода служебные маркеры разработки (`flow`, `Wave`, `Wave`) и связанную “этапную” семантику, чтобы код оставался нейтральным, переносимым и поддерживаемым.

## 2. Текущее состояние (по grep)
- `core` (без `docs`) по ключам `flow|Wave|phase|Wave|stage`: **0 совпадений**.
...
- Удалены `flow`-маркеры из core-комментариев/докстрингов:
...
1. Удалить `roadmap markers` из production-комментариев и докстрингов в `modules/*` и `plugins/*`.
...
4. Добавить CI-проверку, запрещающую новые `roadmap markers` в `core` и в production-слоях.
