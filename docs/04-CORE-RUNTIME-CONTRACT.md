...
### RuntimeModule (домены/инфраструктура, подключаемые приложением)

**Что это:**
- Доменные и инфраструктурные модули, которые подключаются *снаружи* (bootstrap приложения)
- Расширяют систему поверх Core, но не являются частью Core
...
- `automation` — автоматизация (оркестратор; обычно OPTIONAL)
...
- REQUIRED модули: задаются приложением (bootstrap). Core сам список не хранит.
- OPTIONAL модули: задаются приложением (bootstrap) и могут отсутствовать без остановки runtime
