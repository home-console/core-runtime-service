...
**Порядок регистрации (Application Bootstrap, D0):**
- Модули регистрируются приложением через `module_manager.register_module_specs(runtime, specs)` (список задаётся в `app/bootstrap.py` — APP_MODULES).
- **ВАЖНО:** `logger` должен быть первым в списке приложения, так как он нужен для логирования других модулей.
