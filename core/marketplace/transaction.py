...
class UpdateTransactionManager:
        ...
        try:
            ...
            for txn_id, txn_data in pending.items():
                ...
                if txn.state in [TransactionState.SWAPPING, TransactionState.ACTIVATING]:
                    logger.warning(f"Found pending transaction {txn_id} in state {txn.state.value}")
                    self._active_transactions[txn_id] = txn
        except Exception as e:
            logger.error(f"Failed to load pending transactions: {e}")
    
    async def prepare_install(self,
                             plugin_name: str,
                             version: str,
                             archive_path: Path) -> Transaction:
        """
    ...
    def _audit_log(self, txn: Transaction, final_status: str, reason: str = None):
