...
async def register_plugin_control_bindings(
    ...
    async def _admin_restart_plugin_container(name: str = None, body: Any = None, **kw):
        ...
        if not container_exists:
            logger.info("Container %s not found, attempting to ensure it exists", container_name)
        ...
        logger.info("Restarting container %s for plugin %s", container_name, plugin_name)
        restart_result = await orchestration_service.restart_container(container_name, timeout=30.0)
        ...
        if "network" in error_msg.lower() and "not found" in error_msg.lower():
            logger.info("Network error detected, recreating container %s", container_name)
            remove_result = await orchestration_service.remove_container(container_name, force=True)
    ...
    async def _admin_ensure_plugin_container(name: str = None, body: Any = None, **kw):
        ...
        return await orchestration_service.ensure_container(container_name, metadata.container_config)
    ...
    async def _admin_load_plugin_by_name(name: str = None, body: Any = None, **kw):
        ...
        try:
            ...
            ok = await runtime.plugin_manager.load_plugin_by_name(plugin_name, plugins_dir=plugins_dir)
            if ok:
                return {"ok": True}
    ...
    try:
        ...
        logger.warning("Failed to register plugin control HTTP endpoints: %s", e, exc_info=True)
