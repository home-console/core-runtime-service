...
class ProductApiModule(RuntimeModule):
    ...
    async def register(self) -> None:
        ...
        await self.context.services.register("product_api.v1.devices.get_external", _devices_get_external)
        ...
        async def user_credentials_create(**kw: Any) -> Any:
            ...
            if not cred.get("secret_ref") and (cred.get("host") and cred.get("username")):
        ...
        async def user_credentials_get_secret(**kw: Any) -> Any:
            ...
            secret_bytes = bytes.fromhex(hex_secret) if isinstance(hex_secret, str) else hex_secret
        ...
        async def user_credentials_update(**kw: Any) -> Any:
            ...
            secret_str = body.get("secret") or cred.get("secret")
            cred.pop("secret", None)
            secret_bytes = secret_str.encode("utf-8") if secret_str and isinstance(secret_str, str) else None
            return await runtime.service_registry.call(
                "credential.update",
                credential=cred,
                secret=secret_bytes,
        ...
        await services.register("user.v1.credentials.get_secret", user_credentials_get_secret)
        ...
        for method, path, svc, desc in [
            ("GET", "/api/v1/user/credentials", "user.v1.credentials.list", "User: list my credentials"),
            ("POST", "/api/v1/user/credentials", "user.v1.credentials.create", "User: create credential"),
            ("GET", "/api/v1/user/credentials/{credential_id}", "user.v1.credentials.get", "User: get credential"),
            ("GET", "/api/v1/user/credentials/{credential_id}/secret", "user.v1.credentials.get_secret", "User: get credential secret"),
            ("PUT", "/api/v1/user/credentials/{credential_id}", "user.v1.credentials.update", "User: update credential"),
            ("DELETE", "/api/v1/user/credentials/{credential_id}", "user.v1.credentials.delete", "User: delete credential"),
            ("POST", "/api/v1/user/credentials/{credential_id}/connect", "user.v1.credentials.connect", "User: SSH connect by credential"),
        ]:
            self.context.http.register(HttpEndpoint(
                method=method,
                path=path,
    ...
    async def stop(self) -> None:
        for name in (
            "product_api.v1.devices.list", "product_api.v1.devices.get",
            "product_api.v1.devices.set_state", "product_api.v1.devices.get_external",
            "user.v1.credentials.list", "user.v1.credentials.create", "user.v1.credentials.get",
            "user.v1.credentials.get_secret", "user.v1.credentials.update",
            "user.v1.credentials.delete", "user.v1.credentials.connect",
        ):
            try:
