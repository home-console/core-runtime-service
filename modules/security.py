...
def sanitize_for_logging(data: Any, mask: str = "***REDACTED***") -> Any:
    ...
    if isinstance(data, dict):
        ...
        if "authorization" in data.lower() or "bearer" in data.lower() or "oauth" in data.lower():
            # Replace token-like patterns (long alphanumeric strings)
            data = re.sub(r'(bearer|oauth)\s+[a-zA-Z0-9_\-\.]+', r'\1 ***REDACTED***', data, flags=re.IGNORECASE)
            data = re.sub(r'authorization:\s*[^\s]+', 'authorization: ***REDACTED***', data, flags=re.IGNORECASE)
        return data
