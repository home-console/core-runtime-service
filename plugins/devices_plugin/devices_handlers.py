...
async def handle_external_device_discovered(plugin, data: dict) -> None:
    external_id = data.get("external_id")
    if not external_id:
        return

    await plugin.runtime.storage.set("devices_external", external_id, data)


async def handle_external_state(plugin, data: dict) -> None:
    external_id = data.get("external_id")
    ...
    internal_id = await plugin.runtime.storage.get("devices_mappings", external_id)

    if not internal_id:
        return

    device = await plugin.runtime.storage.get("devices", internal_id)
    ...
    await plugin.runtime.storage.set("devices", internal_id, device)
