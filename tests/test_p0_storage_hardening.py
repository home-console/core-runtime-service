...
async def test_crash_safety_large_batch(temp_db):
    ...
    # Write large batch
    for i in range(1000):
        await adapter.set("devices", f"device_{i}", {
            "id": i,
            "status": "active",
            "value": f"data_{i}"
        })
...
async def test_rollback_attack_epoch_regression(temp_db):
    ...
    for i in range(5):
        await secure.secure_set("trust_store", f"key_{i}", {
            "trust_level": i,
            "updated": f"update_{i}"
        })
    ...
    await adapter2.set("_system.meta", "global_epoch", {
        "epoch": 3,
...
async def test_root_hash_tampering_detection(temp_db):
    ...
    # Store initial data
    await secure.secure_set("agent_registry", "agent_1", {
        "name": "Agent 1",
    ...
    await adapter2.set("_system.root_hash", "current", {
        "root_hash": fake_root,
        "epoch": 1,
        "signed_by": "core_key",
    })
