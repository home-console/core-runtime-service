...
async def test_expired_token(runtime):
    ...
    token = jwt_mod.generate_access_token(
        "u1", ["r"], False, secret, expiration_seconds=-1
    )
    assert await jwt_mod.validate_access_token(token, secret) is None
...
async def test_invalid_signature(runtime):
    ...
    bad_secret = "wrong-secret-key-for-tests-32-bytes-min"
    bad_token = jwt_mod.generate_access_token(
        "u2", ["r"], False, bad_secret, expiration_seconds=60
    )
    assert await jwt_mod.validate_access_token(bad_token, secret) is None
...
async def test_malformed_token_handling():
    ...
    assert await jwt_mod.validate_access_token("not-a-jwt", "s") is None
